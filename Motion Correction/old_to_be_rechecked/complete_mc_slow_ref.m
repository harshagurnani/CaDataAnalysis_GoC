function [ best_traj, R, MC_correct ] = complete_mc_slow_ref( RawData, maxD, lambda, varargin )
...
%How many previous timeframes to correct reference used for MC - allow for
%slow activity-dependent changes
refscale = 20;      
% 
...
%     [ best_traj(trial,:), maxlogP(trial) ]  = do_mc_use_viterbi( DX, DY, r, 'uniform', squeeze(floor(RawData(:,:,:,trial,:)*10)/10), squeeze(floor(R(:,:,trial,:)*10)/10),  Tm, nROI, lambda, gamma );
    [ best_traj(trial,:), maxlogP(trial) ]  = do_mc_change_ref( DX, DY, r, 'uniform', squeeze(RawData(:,:,:,trial,:)), R, refscale, Tm, nROI, lambda, gamma );
