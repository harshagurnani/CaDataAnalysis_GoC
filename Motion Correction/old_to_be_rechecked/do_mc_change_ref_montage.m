function [ best_traj, maxlogP ] = do_mc_change_ref_montage( DX, DY, r, initial_st, I, R, refscale, Tm, nROI, lambda, gamma )
%% Viterbi Algorithm
...
tempref = zeros(size(Fy,2), size(Fx,2), (2*DX+1)*(2*DY+1), nROI);
...
%    plane = mod(MC_cycle-1, nROI) + 1;
   ...
   % For each possible displacement( dy(t), dx(t) ):
   ...
   for dy_t = 1:2*DY+1
        ...
        if t<=refscale
            logP1=0;
            for plane=1:nROI
                logP1 = logP1 + sum(sum( probIgivenR( I(Fy+ del_y(dy_t), Fx+ del_x(dx_t), t, plane), ref(:,:,plane), gamma) ) );
            end
        end
...
%            end
           ...
           %%%%% storage.
           % Keep a running sum of last k = 'refscale' MC-corrected intensities

           if t>refscale
              ref = ceil(squeeze( RefHist( :,:, del_index(dy_tminus1, dx_tminus1), :)/refscale )); 
...
%         end
        ...
        % each trajectory
        for plane = 1:nROI
            tempref(:,:,del_index(dy_t, dx_t), plane ) = RefHist(:,:,del_tminus1, plane ) + ...
   ...
   for lastdisp = 1:((2*DX+1)*(2*DY+1))
       if t>refscale
           disp1 = traj_curr( lastdisp, MC_cycle-refscale );
           dx1 = ceil(disp1/(2*DY+1) ) - DX-1;
           dy1 = mod(disp1-1, 2*DY+1) - DY;

           for plane = 1:nROI
               RefHist(:,:, lastdisp, plane) = tempref(:,:,lastdisp,plane) - I(Fy+dy1, Fx+dx1,t-refscale,plane);
           end
       else
           for plane =1:nROI
               RefHist(:,:, lastdisp, plane) = tempref(:,:,lastdisp,plane) ; %- I(Fy+dy1, Fx+dx1,t-refscale,plane);
