...
RefHist = nan(size(Fy,2),size(Fx,2),(2*DX+1)*(2*DY+1), nROI);
tempref = nan(size(Fy,2), size(Fx,2), (2*DX+1)*(2*DY+1));
...
%            end
           ...
           if t>refscale
              ref = RefHist( :,:, del_index(dy_tminus1, dx_tminus1), plane)/refscale; 
              logP1 = sum(sum( probIgivenR( I(Fy+ del_y(dy_t), Fx+ del_x(dx_t), t, plane), ref, gamma) ) );
           end
