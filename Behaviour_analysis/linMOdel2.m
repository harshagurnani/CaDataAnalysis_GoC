...
for jj=1:2
    sn = sess{jj};   
    for jj=1:18
    [ds_vid(jj).(sn), ds_t.(sn)] = resample_ds( as.(sn)(:,jj), VidMI.(sn).whiskpadR_wcam(:,2), dsrate, [0 maxT.(sn)] );
    end
end
...
for s1 = 1:2
    ...
    XNew = [ds_whisk.(sn)', ds_locom.(sn)'];
    for jj=1:18, XNew = [XNew, ds_vid(jj).(sn)']; end
    for jj=1:20, XNew(:,jj) = inpaint_nans(XNew(:,jj)); end
    zz = GramSchmidt(XNew);
    XNew = [zz, ds_puff.(sn) ] ;
    for roi=1:nROI
...
%% Plotting
presp = nan(size(B.AP{1},1)-20,nROI);
Wts.AP = nan(nROI,20);
Wts.baseline = Wts.AP;
...
for s1=1:2
    ...
    for roi=1:nROI
        bestidx = FitInfo.(sn){roi}.Index1SE;
        Wts.(sn)(roi,:) = B.(sn){roi}(1:20,bestidx);
    end
    
    NormWts.(sn) = Wts.(sn)./maxF';
    NormWts2.(sn) = Wts.(sn)./Wts.(sn)(:,1);
end

figure;
for roi=1:nROI
   bestidx = FitInfo.AP{roi}.Index1SE; 
   presp(:,roi) = B.AP{roi}(21:end,bestidx);
   
   plot( B.AP{roi}(21:end,bestidx)+roi*.3); hold on;
end
