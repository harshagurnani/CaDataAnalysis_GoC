function [cc, lags] = get_lagcorr_ds( data1, TS1, data2, TS2, DS_rate,varargin)
%% SYNTAX: [ cc, lags ] = get_lagcorr_ds( data1, SR1, data2, SR2, DS_rate) OR
%                       
% ARGUMENTS:
%   - data1   [n x 1 (FLOAT)]:    
...
% Harsha Gurnani. March 2017
    ...
    % Crop both vectors to same interval ---> first and last timestamp
    ...
    %coeff.
    ind=((~isnan(A1))&(~isnan(A2)));
    nind = nansum(ind);
    [cc, lags] = xcorr(reshape(A1(ind),[nind,1])-nanmean(A1), reshape(A2(ind),[nind,1])-nanmean(A2),'coeff');
