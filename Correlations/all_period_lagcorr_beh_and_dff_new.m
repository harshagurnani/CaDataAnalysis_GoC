function [lags, corrs, beh_period, varargout] =      all_period_lagcorr_beh_and_dff_new( POI, viddata, vidtimes, dff, dff_time, ds_rate, lag_halfrange, varargin )
...
%%%%%%%%% Different modes of correlations
    ...
    tic
    %% Resample once and crop
    
    tinterval = [max(max(dff_time(1,:)),vidtimes(1)), min(min(dff_time(end,:)),vidtimes(end)) ]; % largest shared time interval
    
    % Resample beh data
    [viddata_new, vidtimes_new] = resample_ds(viddata, vidtimes, ds_rate, tinterval );
...
%          dff(:,roi) = inpaint_nans( dff(:,roi),4);
    ...
    [ whisk_mi, whisk_time ] = get_TSOI(viddata_new, vidtimes_new, POI(beh_period,:), extratime);
    
    for roi= 1:n_ROIs
        %%% Crop dff data (or second data series)
       
        [ dff_per, ~ ] = get_TSOI( dff_new(:,roi), dff_newtime(:,roi), POI(beh_period,:), extratime);                  % Crop in periods of interest
        
        for ww=1:n_periods
          A1 = squeeze(dff_per{ww}); A2 = squeeze(whisk_mi{ww});
...
if n_shuffle > 0
    ...
    for roi= 1:n_ROIs
        ...
        temp_dff =circshift(dff_new, shift, 1);  
        %%% Crop dff data (or second data series)
        [ dff_per, ~ ] = get_TSOI( temp_dff(:,roi), dff_newtime(:,roi), POI(beh_period,:), extratime);
        for ww=1:n_periods
          A1 = squeeze(dff_per{ww}); A2 = squeeze(whisk_mi{ww});
          if length(A1)==length(A2)
          cc = corrcoef(A1, A2);
    ...
    for roi=1:n_ROIs
        for ww=1:n_periods
            shuff_null_levels( roi, 1, ww) = prctile(temp_corrs(:,roi,ww),5);
            shuff_null_levels( roi, 2, ww) = prctile(temp_corrs(:,roi,ww),50);
            shuff_null_levels( roi, 3, ww) = prctile(temp_corrs(:,roi,ww),95);
