function [lags, corrs, whisk_period, varargout ] =  all_period_lagcorr_pairwise_dff( POI, dff, dff_time, ds_rate, lag_halfrange, varargin )
%% SYNTAX: [l, c] = all_period_lagcorr_pairwise_dff( locoperiod, dff_values, dff_times,  Downsampling_rate, MaxLag [, extratime, min_POI_length, n_shuffle, do_plot )
...
if n_shuffle > 0
    ...
    parfor shuffle_id = 1:n_shuffle
        ...
        for roi= 1:n_ROIs
            ...
            temp_dff(:,roi) =circshift(dff(:,roi), shift);  
            %%% Crop dff data (or second data series)
            [ dff_per{roi}, dff_pertime{roi} ] = get_TSOI( temp_dff(:,roi), dff_time(:,roi), POI(whisk_period,:), extratime);
        end
        for roi1 = 1:n_ROIs
        for roi2 = roi1+1:n_ROIs
        for ww=1:n_periods
