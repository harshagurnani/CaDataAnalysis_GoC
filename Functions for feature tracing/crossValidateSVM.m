...
validateattributes(sqrt(nNegativeExamples), {'numeric'}, ...
    {'integer', 'positive'}, 'crossValidateSVM', 'sqrt(nNegativeExamples)')
validateattributes(gridSpacing, {'numeric'}, {'positive', 'integer'}, ...
    'crossValidateSVM', 'gridSpacing')
validateattributes(sectionSize,{'numeric'},{'positive', 'integer', ...
    'size', [1,2]}, 'crossValidateSVM', 'sectionSize')
...
for iSet = 0:9
    ...
    % Generate the SVM with the training data and predict the testing data
    svmModel = fitcsvm(trainingPredictor, trainingClassification);
    svmClassification = predict(svmModel, testPredictor);
