...
nAnimals =  1;%size(SummaryData,1); %1;%
nSess = 1;%size(SummaryData,2); %1;%
...
if ~isempty( SummaryData{an, sess})
    ...
    for rr = 1:length(allCorrRate)
        ...
        [lags, corrs, ~, conf_interval, sig ] = ...
            all_period_lagcorr_pairwise_spikes( [0 spikesT(end)], resFR, spikesT, ...
                        corrPar.smoothdt, allCorrRate(rr), corrPar.maxlag, corrPar.extratime, corrPar.minPer, corrPar.nShuffle, corrPar.plot  );
