...
%     SummaryData{an,set}.Speed = L.beh_all.loco;
      ...
      SummaryData{an,set}.spikes = L.activity_all.spikes;
      SummaryData{an,set}.FR = L.activity_all.spikes;
      SummaryData{an,set}.t = L.activity_all.t;
...
%     SummaryData{an,set}.pwCA = L.CorrAnalysis; 
    allf = dir([ fpath, '\spikes*manual*.mat']);
    try
    L  = load( [fpath, allf(1).name],'gid');
    SummaryData{an,set}.usedROI = L.gid;
    catch
    L  = load( [fpath,'\', allf(1).name],'spk_manual');
    SummaryData{an,set}.usedROI = arrayfun( @(jj) L.spk_manual(jj).roi, 1: length(L.spk_manual));    
    end
    set = set+1;
