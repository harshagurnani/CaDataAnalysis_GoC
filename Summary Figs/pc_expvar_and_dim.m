...
%     disp([num2str(animal), ' ', num2str(session), ' ', num2str(res(1)-pc1_2(1))])
    ...
    cc = SummaryData{animal,session}.PCA.all.soma.eig_val;
    PC.nDim_Spectral = [PC.nDim_Spectral;sum(cc)^2/ sum(cc.^2)];
    
    try 
        aa = nanmean( nanmean( SummaryData{animal,session}.PCA.combined.cval.res, 3),1);
        [~,x]= findpeaks( aa, 'NPeaks',1);
        PC.nDim_Peak = [PC.nDim_Peak; x ];
    catch
        PC.nDim_Peak = [PC.nDim_Peak; NaN ];
    end
    if ~(any(pc1_2cv)<0)
...
%%
figure;hold on;
for jj=1:length(PC.nDim_Peak),
     plot([1 2], [PC.nDim_Peak(jj), PC.nDim_Spectral(jj)] ,'o','markerfacecolor','b','color','b','linewidth',1,'markersize',4) 
     plot([1 2], [PC.nDim_Peak(jj), PC.nDim_Spectral(jj)] , 'k','linewidth',0.3) 
end
bb=bar(1,nanmean(PC.nDim_Peak),0.3);bb.FaceAlpha=0.5;bb.FaceColor=[0 0 1];
bb=bar(2,nanmean(PC.nDim_Spectral),0.3);bb.FaceAlpha=0.5;bb.FaceColor='k';
xticks([1 2])
xticklabels({'Peak', 'Spectral'})
